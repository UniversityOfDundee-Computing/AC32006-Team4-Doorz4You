...
function getTeamJobsHandler(PDO $pdo) {
    ...
    $stmnt = $pdo->prepare("SELECT * from joballocation where AllocatedTeam in (SELECT TeamID from teamemployeelist where StaffNo = ?) and not (Status like 'COMPLETED' and EndDate < CURDATE()) order by StartDate");
    $stmnt->execute([$staffDetails[0]['staffno']]);
